...
run "provision_peertube" {
  ...
  variables {
    ...
    dns_hostname = "${run.setup_tests.stack_prefix}-peertube-test.test.patterns.ordinaryexperts.com"
    dns_route53_hosted_zone_name = "test.patterns.ordinaryexperts.com"
    ses_create_domain_identity = false
    stack_name = "${run.setup_tests.stack_prefix}-peertube-test"
  }

  # Check that the DNS Site URL output is correct
  assert {
    condition     = output.dns_site_url_output ==  "https://${run.setup_tests.stack_prefix}-peertube-test.test.patterns.ordinaryexperts.com"
